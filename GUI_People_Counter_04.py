...
def visualizar():
    global inicio, cap, cap2, frame, model, class_names, counter, personas, id_matrix, id_matrix2, Start_Line, max_cap, hora_actual, url, url2
    ...
    if cap2 is not None:
        ...
        if ret2 == True:
            results2 = model.track(frame2, verbose=True, agnostic_nms=True, persist=True, conf = 0.10)
            ...
            if results2[0].boxes.id is not None:
                ...
                confidences2 = results2[0].boxes.conf.cpu().numpy()

                for box2, id2, class_id2, confidence2 in zip(boxes2, ids2, class_ids2, confidences2):
                    x12, y12, x22, y22 = box2
                    center_x2 = int((x12 + x22) / 2)
                    center_y2 = int((y12 + y22) / 2)

                    if class_id2 == 0 or class_id2 ==1: 
                        cv2.rectangle(frame2, (x12, y12), (x22, y22), (0, 255, 255), 4)
                        # cv2.putText(frame, f"ID #{id} {class_names[class_id]} {confidence:.2f}", (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2,)
                        cv2.circle(frame2, (center_x2, center_y2), 5, (0, 255, 255), -1)

                        id_exists2 = False
                        for row2 in id_matrix2:
                            if row2[0] == id2:
                                id_exists2 = True
                                
                                if center_x2 >= Start_Line and not row2[1]: #  |. -> .|
                                    row2[1] = True
                                    counter += 1
                                    personas += 1
                                    guardar_valor_personas(counter)   
                                if center_x2 <= Start_Line and row2[1]:  # .|  ->  |.
                                    row2[1] = False
                                    counter -= 1                  
                                    guardar_valor_personas(counter)   
                        if not id_exists2:
                            if center_x2 <= Start_Line:
                                # print("Persona Entrando")
                                id_matrix2.append([id2, False])

                            if center_x2 >= Start_Line:
                                # print("Persona Saliendo")
                                id_matrix2.append([id2, True])
                        center_x2 = 0
                        center_y2 = 0
            # cv2.line(frame2, (Start_Line, 0), (Start_Line, height), (255, 255, 255), 3)
            # cv2.line(frame, (0, Start_Line), (width, Start_Line), (255, 255, 255), 3)
            # frame = cv2.rotate(frame, cv2.ROTATE_90_COUNTERCLOCKWISE)
